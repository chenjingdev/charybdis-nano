...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [0] = LAYOUT(KC_Q, KC_W, KC_E, KC_R, KC_T, KC_Y, KC_U, KC_I, KC_O, KC_P, MT(MOD_LCTL, KC_A), MT(MOD_LALT, KC_S), MT(MOD_LGUI, KC_D), MT(MOD_LSFT, KC_F), KC_G, KC_H, MT(MOD_RSFT, KC_J), MT(MOD_RGUI, KC_K), MT(MOD_RALT, KC_L), MT(MOD_RCTL, KC_QUOT), LT(4, KC_Z), KC_X, KC_C, KC_V, KC_B, KC_N, KC_M, KC_COMM, KC_DOT, LT(4, KC_SLSH), LT(3, KC_CAPS), LT(2, KC_SPC), LT(1, KC_TAB), LT(6, KC_ENT), LT(5, KC_BSPC)),
...
// J + K => (   K + L => )
const uint16_t PROGMEM jk_open_combo[]  = {MT(MOD_RSFT, KC_J), MT(MOD_RGUI, KC_K), COMBO_END};
const uint16_t PROGMEM kl_close_combo[] = {MT(MOD_RGUI, KC_K), MT(MOD_RALT, KC_L), COMBO_END};
// J(ㅓ) + K(ㅏ) + L(ㅣ) => \(backslash)
const uint16_t PROGMEM jkl_combo[] = {MT(MOD_RSFT, KC_J), MT(MOD_RGUI, KC_K), MT(MOD_RALT, KC_L), COMBO_END};
